...
class DeleteQueryTest {
    ...
    private Table setupTestTable() throws IOException {
        ...
        Optional<Table> optionalTable = schemaManager.loadSchemas().stream().filter(p -> p.getName().equals("test")).findAny();
        if (optionalTable.isEmpty()) {
            throw new IllegalArgumentException("Table '" + optionalTable.get().getName() + "' does not exist");
        }
